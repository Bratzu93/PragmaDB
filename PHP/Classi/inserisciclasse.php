...
END;
			...
			$utilf=mysqli_real_escape_string($conn,$utilf);
			$diagf=mysqli_real_escape_string($conn,$diagf);
			...
			$query1=mysqli_query($query1,$conn) or fail("Query fallita: Inserimento Classe Fallito - ".mysqli_error($conn));
			$queryCod="SELECT c.CodAuto
						FROM Classe c
						WHERE c.PrefixNome='$prefixPadre"."$nomef'";
			$queryCod=mysqli_query($queryCod,$conn) or fail("Query fallita: Classe non trovata nel DB - ".mysqli_error($conn));
			...
			if($num_cluf>0){
				...
				$query2=mysqli_query($query2,$conn) or fail("Query fallita: Inserimento Eredita Da Fallito - ".mysqli_error($conn));
			}
			if($num_cldf>0){
				$query3="CALL insertEreditataDa('$cod','$cldf')";
				$query3=mysqli_query($query3,$conn) or fail("Query fallita: Inserimento Ereditano Da Lei Fallito - ".mysqli_error($conn));
			}
			if($num_cltf>0){
				$query4="CALL insertRelazioneA('$cod','$cltf')";
				$query4=mysqli_query($query4,$conn) or fail("Query fallita: Inserimento IN Fallito - ".mysqli_error($conn));
			}
			if($num_clqf>0){
				$query5="CALL insertRelazioneDa('$cod','$clqf')";
				$query5=mysqli_query($query5,$conn) or fail("Query fallita: Inserimento OUT Fallito - ".mysqli_error($conn));
			}
			if($num_requif>0){
				$query6="CALL insertClasseRequisiti('$cod','$requif')";
				$query6=mysqli_query($query6,$conn) or fail("Query fallita: Inserimento Requisiti Correlati Fallito - ".mysqli_error($conn));
			}
