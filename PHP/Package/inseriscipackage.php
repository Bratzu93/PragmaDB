...
END;
			...
			$query1=$query1."'P')";
			$query1=mysqli_query($query1,$conn) or fail("Query fallita: Inserimento Package Fallito - ".mysqli_error($conn));
			$queryCod="SELECT p.CodAuto
						FROM Package p
						WHERE p.PrefixNome='$prefixPadre"."$nomef'";
			$queryCod=mysqli_query($queryCod,$conn) or fail("Query fallita: Package non trovato nel DB - ".mysqli_error($conn));
			...
			if($num_pkgf>0){
				...
				$query2=mysqli_query($query2,$conn) or fail("Query fallita: Inserimento Package Correlati Fallito - ".mysqli_error($conn));
			}
			/*if($num_requif>0){
				$query3="CALL insertPackageRequisiti('$cod','$requif')";
				$query3=mysqli_query($query3,$conn) or fail("Query fallita: Inserimento Requisiti Correlati Fallito - ".mysqli_error($conn));
			}*/
