...
if(empty($_SESSION['user'])){
	...
	header('Content-Disposition: attachment; filename="packageClassiST-BE.tex"');
	header('Expires: 0');
	...
	$query="SELECT p1.CodAuto, p1.PrefixNome, p1.UML, p1.Descrizione, p1.Padre, p2.PrefixNome, p2.Nome
			...
			WHERE LEFT(p1.PrefixNome,15)='Premi::Back-End'
...
\\begin{figure}[h]
\\centering
\\nogloxy{\\includegraphics[scale=0.4,keepaspectratio]{../definizioneDiProdotto/diagrammi/package/{{$row[2]}}.pdf}}
\\caption{\\nogloxy{{$row[1]}}}
\\end{figure}
\\FloatBarrier
END;
...
END;
		...
		$queryRelated="SELECT p.PrefixNome, p.Nome, p.Descrizione
					   ...
					   ORDER BY PrefixNome";
		$related=mysqli_query($queryRelated,$conn) or fail("Query fallita: ".mysqli_error($conn));
...
END;
		...
		$subpack=mysqli_query($querySubPack,$conn) or fail("Query fallita: ".mysqli_error($conn));
...
END;
		...
		$classi=mysqli_query($queryClassi,$conn) or fail("Query fallita: ".mysqli_error($conn));
