...
END;
					...
					$query1=mysqli_query($query1,$conn) or fail("Query fallita: Modifica Test Fallita - ".mysqli_error($conn));
					if(($tipof=="Unita") && ($modifica_met==true)){
						$query2="CALL modifyTestMetodi('$id','$metf')";
						$query2=mysqli_query($query2,$conn) or fail("Query fallita: Modifica Metodi Correlati Fallita - ".mysqli_error($conn));
					}
...
END;
		...
		$queryTipo="SELECT t.Tipo
					FROM Test t
					WHERE t.CodAuto='$id'";
		$tipo=mysqli_query($queryTipo,$conn) or fail("Query fallita: ".mysqli_error($conn));
