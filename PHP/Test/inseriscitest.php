...
END;
			...
			$query1=mysqli_query($query1,$conn) or fail("Query fallita: Inserimento Test Fallito - ".mysqli_error($conn));
			if($num_metf>0){
				$queryCod="SELECT t.CodAuto
						   FROM Test t
						   WHERE t.Tipo='$tipof' AND t.Descrizione='$descf'
						   ORDER BY t.Time DESC";
				$queryCod=mysqli_query($queryCod,$conn) or fail("Query fallita: Test non trovato nel DB - ".mysqli_error($conn));
				...
				$query2=mysqli_query($query2,$conn) or fail("Query fallita: Inserimento Metodi Correlati Fallito - ".mysqli_error($conn));
			}
			$title="Test Inserito";
			startpage_builder($title);
echo<<<END
...
END;
		...
		while($row=mysqli_fetch_row($met)){
			if($row[0]!=null){
				...
				$par=mysqli_query($query_par,$conn) or fail("Query fallita: ".mysqli_error($conn));
echo<<<END
