...
END;
	...
	$padri=mysqli_query($queryEredita,$conn) or fail("Query fallita: ".mysqli_error($conn));
...
END;
	...
	$querySubClassi="SELECT c.PrefixNome, c.Nome
					 ...
					 ORDER BY c.PrefixNome";
	$subclassi=mysqli_query($querySubClassi,$conn) or fail("Query fallita: ".mysqli_error($conn));
...
END;
	...
	$queryRelationsOUT="SELECT c.PrefixNome, c.Nome, c.Descrizione
						...
						WHERE r.Da='$riga[0]'
						ORDER BY c.PrefixNome";
	$in=mysqli_query($queryRelationsIN,$conn) or fail("Query fallita: ".mysqli_error($conn));
	$out=mysqli_query($queryRelationsOUT,$conn) or fail("Query fallita: ".mysqli_error($conn));
